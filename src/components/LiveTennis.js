...
const LiveTennis = () => {
  ...
  const fetchData = () => {
    axios
      ...
      .then((res) => {
        res.data.matches.map(m => { console.log(m.Tournament) })
        setDanger("");
        setData(res.data.matches);
        setLimitRequestLeft(res.headers["x-ratelimit-requests-remaining"]);
        // console.log(limitRequestLeft);
        setTournaments(res.data.matches.map(match => match.Tournament)
  ...
  return (
    <div className="wizard">
      ...
      <div className="container-fluid border border-primary mx-auto rounded mt-12">

        <div className="row">
          <div className="col-9 mx-right">
            <div>
              {danger ? (
          ...
          <div className="col-3 mx-right border border-light">
            <ul>
              {tournaments.length < 1 ? (
                ...
                tournaments.map((m, i) => (
                  <li key={i}><p className="text-light"><input type="checkbox" />&nbsp;{m}</p></li>
                ))
              )}
            </ul>
          </div>
